...
export default function HomeScreen({navigation, route}) {
  ...
  return (
    <View style={styles.container}>
      {/* לא בטלוויזיה. זהו WebView במסך מלא מאחורי כל התוכן, ובתוכו שלושה
          כתמים בגודל 82vmax (גדולים מהמסך) עם filter:blur(60px) ושלוש
          אנימציות אינסופיות. בטלפון זה עובר; בטלוויזיה — מעבד חלש, בלי
          האצת blur בחומרה, ורזולוציה גבוהה שמכפילה את שטח הטשטוש — זה
          חונק את התהליך הראשי מרגע שמסך הבית עולה.
          דווח מהשטח: נכנסים לאפליקציה בטלוויזיה, השלט מפסיק להגיב אחרי
      ...
      {/* isFocused ולא רק detailItem: זהו Modal אמיתי, ובאנדרואיד Modal נפתח
          בחלון נפרד מעל הכל — כולל מעל מסך הנגן. הפריט נשאר פתוח בכוונה
          כשעוברים לנגן (כדי שהצופה יחזור לבחור את הפרק הבא), אבל אחרי מעבר
          דרך החלון הצף סדר החלונות התהפך והוא צף מעל הסרט: הצופה עשה "מסך
          מלא" מהחלון הצף וקיבל את דף הפרטים, ורק לחיצה על ✕ חשפה את הנגן
          שהיה שם כל הזמן.
          כאן רק *מפסיקים לצייר* אותו כשהמסך אינו הפעיל — ה-state נשמר,
          ולכן ההתנהגות של "לחזור ולבחור פרק הבא" נשארת בדיוק כשהייתה. */}
      {detailItem && isFocused && (
        <MovieDetailModal
          item={detailItem}
          allMovies={movies}
